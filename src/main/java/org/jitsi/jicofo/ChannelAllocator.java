...
public class ChannelAllocator implements Runnable
{
    ...
    { String clientName = clientAuthenticationAuthority.getClientNameFromSessionMap(meetConference.getRoomName());
        ...
        {
        	logger.error("Client : "+clientName+" :", e);
    ...
    private void discoverFeaturesAndInvite()
    {
        ...
        logger.info(
        		" Client : "+ clientName+ " : " + address + " has bundle ? " + newParticipant.hasBundleSupport() );
        ...
        try
        {
            ...
            if (offer == null)
            {
                logger.info(" Client : "+ clientName+" : Channel allocation cancelled for " + address);
                return;
            }
        }
        ...
        */
        ...
        if (chatRoom == null)
        {
            // Conference disposed
            logger.info(
            		" Client : "+ clientName+" : Expiring " + address + " channels - conference disposed");
        ...
        else if (meetConference.findMember(address) == null)
        {
            ...
            logger.info(
            		" Client : "+ clientName+ " : Expiring " + address + " channels - participant has left");
        ...
        else
        {
            ...
            if (!ack)
            {
                // Failed to invite
                logger.info(
                		" Client : "+ clientName +" : Expiring " + address + " channels - no RESULT for "
        ...
        throws OperationFailedException
    {
        ...
        while (!colibriConference.isDisposed())
        {
            ...
            {
                logger.error( " Client : "+ clientName +
        ...
        for (ContentPacketExtension cpe : contents)
        {
                    ...
                    : colibriContent.getSctpConnections())
            {
                ...
                if (useBundle)
                {
                    ...
                    {
                        logger.error(
                        		" Client : "+ clientName+" : No bundle for " + sctpConn.getChannelBundleId());
                        continue;
                    }
