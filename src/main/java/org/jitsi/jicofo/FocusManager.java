 ...
 * @author Pawel Domas
 */
public class FocusManager implements JitsiMeetConference.ConferenceListener, RegistrationStateChangeListener {
	/**
	 ...
	 */
	public static final long DEFAULT_IDLE_TIMEOUT = 150000;
	...
	/**
	 * The name of configuration property that specifies XMPP domain of the
	 * focus user.
	 */
	...
	/**
	 * Indicates if graceful shutdown mode has been enabled and no new
	 * conference request will be accepted.
	 */
	...
	/**
	 * Handler that takes care of Client Authentication and corresponding
	 * session.
	 */
	...
	public void start() throws Exception {
		...
		ProviderManager.getInstance().addExtensionProvider(LogPacketExtension.LOG_ELEM_NAME,
				LogPacketExtension.NAMESPACE, new LogExtensionProvider());
	 ...
	 *
	 * @param room
	 *            the name of MUC room for which new conference has to be
	 *            allocated.
	 * @param properties
	 ...
	 * @throws Exception
	 *             if for any reason we have failed to create the conference
	 */
	public synchronized boolean conferenceRequest(String room, Map<String, String> properties) throws Exception {
	 ...
	 * 
	 * @param room
	 *            name of the MUC room of Jitsi Meet conference.
	 * @param properties
	 *            configuration properties, see {@link JitsiMeetConfig} for the
	...
	private void createConference(String room, Map<String, String> properties) throws Exception {
		...
		JitsiMeetConference conference = new JitsiMeetConference(room, focusUserName, protocolProviderHandler, this,
				config, globalConfig);
		...
		String clientName = clientAuthenticationAuthority.getClientNameFromSessionMap(room);
		logger.info("Client : " +clientName + " : Created new focus for " + room + "@" + focusUserDomain + " conferences count: "
		...
		try {
			...
			logger.info("Client : " +clientName + " : Exception while trying to start the conference", e);
	 ...
	 * 
	 * @param roomName
	 *            full MUC room name to destroy.
	 * @param reason
	 *            optional reason string that will be advertised to the users
	 *            upon exit.
	 */
	public synchronized void destroyConference(String roomName, String reason) {
		...
		if (conference == null) {
			String clientName = clientAuthenticationAuthority.getClientNameFromSessionMap(roomName);
			logger.error("Client :" +clientName+ " : Unable to destroy the conference - not found: " + roomName);
			return;
		}
	...
	public synchronized void conferenceEnded(JitsiMeetConference conference) {
		...
		AuthenticationSession clientSession = clientAuthenticationAuthority.getSessionFormSessionMap(roomName);

		if (clientSession != null) {

			clientAuthenticationAuthority.destroySession(roomName);
		}

		logger.info("Client : " + clientSession.getClientName() + " : Disposed conference for room: " + roomName
		...
		EventAdmin eventAdmin = FocusBundleActivator.getEventAdmin();
		if (eventAdmin != null) {
			eventAdmin.sendEvent(EventFactory.focusDestroyed(conference.getId(), conference.getRoomName()));
		}
	 ...
	 *
	 * @param roomName
	 *            the name of MUC room for which we want get the
	 *            {@code JitsiMeetConference} instance.
	 * @return the {@code JitsiMeetConference} for the specified
	 *         {@code roomName} or {@code null} if no conference has been
	 *         allocated yet
	 */
	...
	public interface FocusAllocationListener {
		 ...
		 * 
		 * @param roomName
		 *            the name of the conference room for which focus has been
		 *            destroyed.
		 */
	 ...
	 *            operation set class.
	 * @param <T>
	 *            the class of Operation Set to be returned
	...
	class FocusExpireThread {
		...
		public FocusExpireThread() {
			timeout = FocusBundleActivator.getConfigService().getLong(IDLE_TIMEOUT_PROP_NAME, DEFAULT_IDLE_TIMEOUT);
		}
