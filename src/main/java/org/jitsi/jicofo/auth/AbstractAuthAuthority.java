...
import org.jitsi.jicofo.log.EventFactory;
...
public abstract class AbstractAuthAuthority implements AuthenticationAuthority, FocusManager.FocusAllocationListener {
	 ...
	 */
	private List<AuthenticationListener> authenticationListeners = new CopyOnWriteArrayList<AuthenticationListener>();
	...
	public AbstractAuthAuthority() {
		...
		authenticationLifetime = cfg.getLong(AUTHENTICATION_LIFETIME_PNAME, DEFAULT_AUTHENTICATION_LIFETIME);
		logger.info("Authentication lifetime: " + authenticationLifetime);

		disableAutoLogin = cfg.getBoolean(AuthBundleActivator.DISABLE_AUTOLOGIN_PNAME, false);
		if (disableAutoLogin) {
	 ...
	 *
	 * @param machineUID
	 *            unique machine identifier for new session.
	 * @param authIdentity
	 *            authenticated user's identity name that will be used in new
	 *            session.
	 * @param roomName
	 *            the name of the conference for which the session will be
	 *            created
	 * @param properties
	 *            the list of authentication properties provided during
	 *            authentication which will be sent in 'authentication session
	 *            created' event. This is authentication provider depended and
	 *            can be left empty.
	 *
	 * @return new <tt>AuthenticationSession</tt> for given parameters.
	 */
	protected AuthenticationSession createNewSession(String machineUID, String clientKey, String roomName,
			Map<String, String> properties, String clientName) {
		synchronized (syncRoot) {
			AuthenticationSession session = new AuthenticationSession(machineUID, createNonExistingUUID().toString(),
					clientKey, roomName, clientName);

			/*
			 * The key being used in the map is combination of room name and client key
			*/
			authenticationSessions.put(roomName, session);

			logger.info("Client : " + clientName + " : Authentication session created for " + clientKey + " SID: "
	...
	/**
	 * Finds <tt>AuthenticationSession</tt> for given MUID and user's
	 * authenticated identity name.
	 *
	 * @param machineUID
	 *            unique machine identifier used to distinguish between sessions
	 *            for the same user on different machines.
	 * @param authIdentity
	 *            authenticated user's identity name(usually login or email)
	 *
	 * @return <tt>AuthenticationSession</tt> for given MUID and user's
	 *         authenticated identity name
	 */
	protected AuthenticationSession findSessionForIdentity(String roomName, String clientKey) {

		if (StringUtils.isNullOrEmpty(clientKey) || StringUtils.isNullOrEmpty(roomName)) {
			return null;
		}
		synchronized (syncRoot) {
			for (AuthenticationSession session : authenticationSessions.values()) {
				if (session.getUserIdentity().equals(clientKey) && session.getRoomName().equals(roomName)) {
					return session;
				}
			}
			return null;
	...
	/**
	 * Finds <tt>Client name</tt> for given room name.
	 *
	 * @param roomName
	 *            Room Name for which authentication session will be searched.
	 *
	 * @return <tt>ClientName</tt> for given Room Name or <tt>null</tt> if not
	 *         found.
	 */

	protected String findClientNameForRoomName(String roomName) {
		if (StringUtils.isNullOrEmpty(roomName)) {
			return null;
		}
		synchronized (syncRoot) {
			for (AuthenticationSession session : authenticationSessions.values()) {
				if (session.getRoomName().equals(roomName)) {
					return session.getClientName();
				}
			}
			return null;
		}
	...
	/**
	 * Finds <tt>Client Name</tt> for given clientKey and roomName.
	 *
	 * @param clientKey
	 * @return <tt>ClientName</tt> for given clientKey and roomName
	 */

	protected AuthenticationSession findSessionForRoomName(String roomName) {
		if (StringUtils.isNullOrEmpty(roomName)) {
			return null;
		}
		synchronized (syncRoot) {
	 ...
	 *
	 * @param jabberId
	 *            Jabber ID for which authentication session will be searched.
	 *
	 ...
	 *
	 * @param sessionId
	 *            unique authentication session identifier string that will be
	 *            used in the search.
	 */
	 ...
	 */
	public String destroySession(String roomName) {
		synchronized (syncRoot) {
			AuthenticationSession session = authenticationSessions.get(roomName);

			if (session == null)
				return null;

			if (authenticationSessions.remove(roomName) != null) {

				logger.info(
						"Client : " + findClientNameForRoomName(roomName) + " : Authentication removed: " + session);

				// Generate "authentication session destroyed" event
				logEvent(EventFactory.authSessionDestroyed(session.getSessionId()));
				return session.getRoomName();
		...
		synchronized (syncRoot) {
			...
			while (sessionIterator.hasNext()) {
				...
				if (roomName.equals(session.getRoomName())) {
					logger.info("Client : " + findClientNameForRoomName(roomName)
	 ...
	 * 
	 * @param l
	 *            the <tt>AuthenticationListener</tt> to be added to listeners
	 *            list.
	 */
	@Override
	 ...
	 * 
	 * @param l
	 *            the <tt>AuthenticationListener</tt> that will be removed from
	...
	protected void notifyUserAuthenticated(String userJid, String identity, String sessionId) {
	 ...
	 * authentication session's identifier during authentication request
	 * handling process(
	 * {@link #processAuthentication(ConferenceIq, ConferenceIq)}).
	 *
	 * @param query
	 *            <tt>ConferenceIq</tt> that contains(or not) session id for the
	 *            verification.
	 *
	 * @return <tt>null</tt> if no problems where discovered during verification
	 *         process or XMPP error that should be returned as response to
	 *         given <tt>query</tt>.
	 */
	 ...
	 * order to authenticate JID found in the 'from' field of the request with
	 * given <tt>AuthenticationSession</tt>(based on supplied session-id). When
	 * called it means that the request sent by <tt>peerJid</tt> is valid and
	 * has been authenticated for given <tt>session</tt>. Authentication process
	 * is specific to {@link AuthenticationAuthority} implementation.
	 *
	 * @param session
	 *            <tt>AuthenticationSession</tt> which corresponds to the
	 *            session id supplied in the request.
	 * @param peerJid
	 *            user's JID that will be associated with
	 *            <tt>AuthenticationSession</tt>.
	 * @param response
	 *            the response instance that will be sent back to the user. It
	 *            will be updated with required information to describe the
	 *            session.
	 */
	protected void authenticateJidWithSession(AuthenticationSession session, String peerJid, ConferenceIq response) {
	 ...
	 * logout process. If this step is not required then <tt>null</tt> is
	 * returned.
	 *
	 * @param sessionId
	 *            authentication session identifier string.
	 */
	...
	public void start() {
		 ...
		 * expireTimer.scheduleAtFixedRate( new ExpireTask(),
		 * EXPIRE_POLLING_INTERVAL, EXPIRE_POLLING_INTERVAL);
		 */

		this.focusManager = ServiceUtils.getService(AuthBundleActivator.bundleContext, FocusManager.class);
