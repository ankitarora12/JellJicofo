	...
	public void start(BundleContext bundleContext) throws Exception {
		...
		if (!StringUtils.isNullOrEmpty(loginUrl)) {
			...
			if (loginUrl.toUpperCase().startsWith("XMPP:")) {
				authAuthority = new XMPPDomainAuthAuthority(loginUrl.substring(5));
			} else {
				String logoutUrl = cfg.getString(LOGOUT_URL_PNAME);

				authAuthority = new ShibbolethAuthAuthority(loginUrl, logoutUrl);
			}

		} else {
			authAuthority = JitsiMeetGlobalConfig.clientAuthenticationAuthority;
		}
